...
# 检查构建结果
if [ -f "dist/鼠标点击器" ]; then
    echo "✅ 构建成功！"
    echo "📁 可执行文件位置: $(pwd)/dist/鼠标点击器"
    echo "📊 文件大小: $(du -h dist/鼠标点击器 | cut -f1)"
    ...
    echo "======================================"
    echo "📋 使用说明:"
    echo "  1. 双击运行: dist/鼠标点击器"
    echo "  2. 或命令行运行: ./dist/鼠标点击器"
    echo "  3. 首次运行需要授予辅助功能权限"
    echo "     (系统偏好设置 > 安全性与隐私 > 隐私 > 辅助功能)"
    echo ""
    echo "📦 分发说明:"
    echo "  - 可以直接分发 dist/鼠标点击器 文件"
